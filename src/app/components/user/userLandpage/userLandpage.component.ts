...
export class UserLandpageComponent implements OnInit {
  ...
  constructor(private taskService: TaskService, private projectService: ProjectService, private relationsProjectsService: RelationsProjectsService) { }
  ...
  async ngOnInit() {
    ...
    forkJoin([
      ...
      const relationsProjectsList: Relation[] = await (relationsResponse as any).relations_projects;
    ...
    this.relationsProjectsList.forEach((relation: Relation) => {
      this.taskList.forEach((task: Task) => {
        this.projectList.forEach((project: Project) => {
          ...
          if (relation.projectId == project.projectId && relation.taskId == task.taskId) {
